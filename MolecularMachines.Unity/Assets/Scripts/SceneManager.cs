...
public class SceneManager : MonoBehaviour
{
    ...
    internal void UpdateProteinColor(int i, object toUnity)
    {
    ...
    public void UpdateProteinInstanceLocRot(int index, Vector3 position, Quaternion rotation)
    {
        ProteinInstancePositions[index] = position;
        ProteinInstanceRotations[index] = Helper.QuanternionToVector4(rotation);
    }

    public void UpdateProteinAtoms(int index, IEnumerable<Vector4> atoms)
    {
        if (index > ProteinAtomStart.Count) { throw new Exception("index > ProteinAtomStart.Count (" + index + " > " + ProteinAtomStart.Count + ")"); }
        if (index > ProteinAtomCount.Count) { throw new Exception("index > ProteinAtomCount.Count (" + index + " > " + ProteinAtomCount.Count + ")"); }

        var start = ProteinAtomStart[index];
        var count = ProteinAtomCount[index];
        var upperLimit = start + count - 1;

        if (upperLimit > ProteinAtoms.Count - 1) { throw new Exception("upperLimit > ProteinAtoms.Count-1 (" + upperLimit + " > " + (ProteinAtoms.Count - 1) + ")"); }

        var i = start;
        foreach (var p in atoms)
        {
            if (i > upperLimit) { throw new Exception("count of atoms to update is bigger than ProteinAtomCount"); }

            ProteinAtoms[i] = p;
            i++;
        }

        if (i <= upperLimit) { throw new Exception("too less atoms updated"); }
    }

    public void UpdateProteinColor(int index, Color color)
    {
        ProteinColors[index] = color;
    }
    ...
    public void UploadProteinInstancePositionsRotations()
    {
    ...
    public void UploadAllData()
    {
        ...
        UploadProteinInstancePositionsRotations();
