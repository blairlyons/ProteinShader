...
public static class ColorPaletteGenerator
{
  ...
  */
    ...
    */
    ...
    public static bool testfunction(Vector3 acolor)
    ...
    public static List<Vector3> generate(int colorsCount, afunction checkColor, bool forceMode, int quality, bool ultra_precision = false)
    {
                ...
                for (int i = 0; i < colors.Count; i++)
                {
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        ...
                        if (d > 0)
                        {
                            ...
                            float tmpX = vectors[i][0] + dl * force / d;
                            float tmpY = vectors[i][1] + da * force / d;
                            float tmpZ = vectors[i][2] + db * force / d;
                            vectors[i] = new Vector3(tmpX, tmpY, tmpZ);

                            float tmpX2 = vectors[j][0] - dl * force / d;
                            float tmpY2 = vectors[j][1] - da * force / d;
                            float tmpZ2 = vectors[j][2] - db * force / d;
                            vectors[j] = new Vector3(tmpX2, tmpY2, tmpZ2);
                        }
                        else
                        {
                            // Jitter
                            float tmpX3 = vectors[j][0] + 0.02f - 0.04f * Random.value;
                            float tmpY3 = vectors[j][1] + 0.02f - 0.04f * Random.value;
                            float tmpZ3 = vectors[j][2] + 0.02f - 0.04f * Random.value;
                            vectors[j] = new Vector3(tmpX3, tmpY3, tmpZ3);
                        }
                    }
                }
                // Apply Force
        ...
        else
        {
            ...
            for (int i = 0; i < colorsCount; i++)
            {
                ...
                while (!checkColor2(lab, checkColor))
                {//chroma.lab(lab)
